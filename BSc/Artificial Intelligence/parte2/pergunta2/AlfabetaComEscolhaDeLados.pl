...
oper2(Lista,Points1,Points2,player2, PointsNovos1,PointsNovos2,Y,ListaFinal,N) :-
	...
	((Max is 0,
    Max2 > 0,
	alfabeta(Lista,Points1,Points2,Op-Es-Pontos1-Pontos2,N),
...
continuaJogadas(Ei,Pontos1,Pontos2,N) :-
	...
	write('Empate'))));
    (\+terminal(Es,PointsNovos1,PointsNovos2),
    alfabeta(Es,PointsNovos1,PointsNovos2,Op-Es2-PontosNovos3-PontosNovos4,N),
...
alfabeta(Ei,Points1,Points2,Opf,TimeToStop) :-
	findall(Vc-Op-Es-PointsNovos1-PointsNovos2, (oper(Ei,Points1,Points2,player1,PointsNovos1,PointsNovos2,Op,Es,_), alfabeta_min(Es,Vc,1,-10000,10000,TimeToStop,PointsNovos1,PointsNovos2)), L),
	escolhe_max(L,Opf).



alfabeta_min(Ei,Val,P,_,_,TimeToStop,Points1,Points2) :- 
	(P is TimeToStop;
	terminal(Ei,Points1,Points2)),
	valorCheck(Ei,P,Points1,Points2,Val), !.

alfabeta_min(Ei,Val,P,Alfa,Beta,TimeToStop,Points1,Points2) :- 
	P1 is P+1, jogador(P1,J),
	V is 10000,
	findall(Es-PointsNovos1-PointsNovos2, oper(Ei,Points1,Points2,J,PointsNovos1,PointsNovos2,_,Es,_), L),
	processa_lista_min(L, P1, V, Alfa, Beta, Val,TimeToStop), !.

processa_lista_min([], _, V, _, _, V,_).
processa_lista_min([H-Points1-Points2|T], P, V, A, B, V1,TimeToStop) :-
	alfabeta_max(H, V2, P, -10000, 10000,TimeToStop,Points1,Points2),
	min(V, V2, V3),
	(V3 < A, V1 is V3; min(B, V3, B1), processa_lista_min(T, P, V3, A, B1, V1,TimeToStop)).

min(A,B,A) :- A < B, !.
min(_, B, B).

alfabeta_max(Ei,Val,P,_,_,TimeToStop,Points1,Points2) :- 
	(P is TimeToStop;
	terminal(Ei,Points1,Points2)),
	valorCheck(Ei,P,Points1,Points2,Val), !.

alfabeta_max(Ei,Val,P,Alfa,Beta,TimeToStop,Points1,Points2) :- 
	P1 is P+1, jogador(P1,J),
	V is -10000,
	findall(Es-PointsNovos1-PointsNovos2, oper(Ei,Points1,Points2,J,PointsNovos1,PointsNovos2,_,Es,_), L),
	processa_lista_max(L, P1, V, Alfa, Beta, Val,TimeToStop), !.

processa_lista_max([], _, V, _, _, V,_).
processa_lista_max([H-Points1-Points2|T], P, V, A, B, V1,TimeToStop) :-
	alfabeta_min(H, V2, P, -10000, 10000,TimeToStop,Points1,Points2),
	max(V, V2, V3),
	(V3 >= B, V1 is V3; max(A, V3, A1), processa_lista_max(T, P, V3, A1, B, V1,TimeToStop)).
